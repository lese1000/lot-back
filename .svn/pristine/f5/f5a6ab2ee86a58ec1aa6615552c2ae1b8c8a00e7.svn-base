...
<body class="gray-bg">
	...
	<div class="row">
	<div class="col-sm-12">
	<form:form id="searchForm" modelAttribute="orderLotteryNumVo" action="${ctx}/lottery/pOrderReport/list" method="post" class="form-inline">
		<div class="form-group">
			...
			<span>记录时间：</span>
				<input id="createDateStart" name="createDateStart" type="text" maxlength="20" class="laydate-icon form-control layer-date input-sm"
					value="<fmt:formatDate value="${orderLotteryNumVo.createDateStart}" pattern="yyyy-MM-dd HH:mm:ss"/>"/> - 
				<input id="createDateEnd" name="createDateEnd" type="text" maxlength="20" class="laydate-icon form-control layer-date input-sm"
					value="<fmt:formatDate value="${orderLotteryNumVo.createDateEnd}" pattern="yyyy-MM-dd HH:mm:ss"/>"/>
		 </div>	
	</form:form>
	<br/>
	</div>
	...
	<div class="col-sm-12">
		<div class="pull-left" style="width:50%">
			<shiro:hasPermission name="lottery:pOrderReport:export">
	       		<table:exportExcel url="${ctx}/lottery/pOrderReport/export"></table:exportExcel>
	       	</shiro:hasPermission>
	...
	<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		<thead>
			<tr>
				...
				<th  class="sort-column lotteryTypeName">彩票种类</th>
		...
		<tbody>
		<c:forEach items="${dataList}" var="orderLotteryNumVo" varStatus="status">
			<tr>
				<td>${ status.index + 1}</td>  
				<td>
						${orderLotteryNumVo.lotteryTypeName}
				</td>
				<td>
						${orderLotteryNumVo.lotteryNum}
				</td>
				<td>
						${orderLotteryNumVo.totalBettingMoney}
				</td>
				<td>
						${orderLotteryNumVo.totalWinMoney}
				</td>
			</tr>
		</c:forEach>
		</tbody>
